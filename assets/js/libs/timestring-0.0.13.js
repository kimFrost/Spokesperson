...
---------------------------------------**/
		...
		private.create = function() {
			...
			private.internal.devices.ie8 = true; // TEMP TEST

			// Check if MediaElementPlayer.js is available
			if (window.MediaElementPlayer != undefined) {
				private.internal.libs.mediaElementPlayer = MediaElementPlayer;
			}
			...
			if (!private.internal.devices.ie8) {
			...
			else if (private.internal.libs.mediaElementPlayer != null) {
				// Create on fallBackPlayerContainer  player on the render canvas
				...
				$('<video id='+fallbackContainerId+'></video>').appendTo(private.data.fallBackPlayerContainer);

				private.data.fallBackPlayer = new private.internal.libs.mediaElementPlayer('#'+fallbackContainerId, {
					features: ['playpause', 'progress', 'duration', 'volume'],
					defaultVideoWidth: public.options.size.width,
					defaultVideoHeight: public.options.size.height,
					//flashName: 'scripts/flashmediaelement.swf',
					timerRate: 250,
				...
				//console.log(private.data.fallBackPlayer);
			}
