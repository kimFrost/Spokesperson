							...
							if (movie.triggers.length > 0) {
								for (var m=0;m<movie.triggers.length;m++) {
									...
									if (typeof trigger.trigger == "number") {
										//private.log(trigger);
										if (movie.time.current + (movie.time.playbackRate * public.options.fixedUpdateInterval) - trigger.trigger > public.options.fixedUpdateInterval) {
											private.log("trig the time event");
										}

										//private.log(movie.time.playbackRate);
				...
				if (scene.states.draw) {
					...
					for (var m=0;m<private.data.movies.length;m++) {
						...
						if (movie.scene === scene.id && movie.states.draw) {
							...
							private.data.renderViewCtx.drawImage(movie.elem[0], 0, 0, movie.elem.width(), movie.elem.height());
						}
					}
				}
			}
