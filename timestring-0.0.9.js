...
---------------------------------------**/
		...
		private.create = function() {
			...
			private.data.bufferContainer = $('<div class="timestring-buffers"></div>').appendTo(private.data.rootElem);
				...
				if (!scene.states.sleep) {
					if (scene.triggers.length > 0) {
						for (var m=0;m<scene.triggers.length;m++) {
							...
							// Last checked time
							//trigger.lastChecked = (Date.now || new Date().getTime)();
						}
					}
							...
							if (movie.triggers.length > 0) {
								...
								for (var m=0;m<movie.triggers.length;m++) {
									...
									if (typeof trigger.trigger == "number") {
										...
										//private.log((Date.now || new Date().getTime)());
										//private.log(movie.time.current);
										//var timeSince = (Date.now || new Date().getTime)() - trigger.lastChecked;
										//private.log(timeSince.toString() + " ms since trigger check");
